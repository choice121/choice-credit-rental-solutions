        ...
        <div className="grid grid-cols-2 md:grid-cols-4 divide-x divide-primary-foreground/10">
          {stats.map((s, i) => (
            <div key={i} className="py-10 px-6 text-center flex flex-col items-center gap-2">
              ...
              <div className="text-4xl font-serif font-bold text-primary-foreground tabular-nums">
                {s.value}
              </div>
    ...
    <div className="flex items-center justify-center gap-3 mt-8">
      <div className="flex -space-x-2">
        {AVATAR_INITIALS.map((init, i) => (
          <div
            key={i}
            className={`w-8 h-8 rounded-full ${AVATAR_COLORS[i]} border-2 border-primary flex items-center justify-center text-white text-[10px] font-bold`}
          >
            {init}
          </div>
        ))}
  ...
  const points = [
    {
      ...
      desc: "Your personal and financial data is handled with bank-level encryption. We never share your information.",
    },
    {
      icon: <TrendingUp className="w-6 h-6" />,
      title: "Data-Driven Strategies",
      desc: "No generic templates. Every strategy is built around current scoring models and your specific denial reasons.",
    },
    {
      icon: <MapPin className="w-6 h-6" />,
      title: "Nationwide Coverage",
      desc: "We serve clients in all 50 states. Wherever you need to move, we have the experience to get you there.",
    },
    {
      icon: <Users className="w-6 h-6" />,
      title: "Dedicated Advisor",
      desc: "You work directly with a real person — not a chatbot or support queue. Unlimited messaging on most packages.",
    },
    {
      icon: <Lock className="w-6 h-6" />,
      title: "Legal & Fully Ethical",
      desc: "Every tactic we use is grounded in the Fair Credit Reporting Act. We don't cut corners — we know the rules cold.",
    },
    {
      icon: <Award className="w-6 h-6" />,
      title: "98% Approval Track Record",
      desc: "Over 500 families housed. We don't count clients who give up — we count clients who move in.",
    },
  ];
      ...
      <div className="container max-w-6xl">
        <div className="grid lg:grid-cols-2 gap-16 items-start">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            {points.map((p, i) => (
              <div key={i} className="flex gap-4 p-4 rounded-xl hover:bg-muted/60 transition-colors">
